   ...
   const testAreas = [
    {
      ...
      score:
        studentProfile.testScores.verbal && typeof studentProfile.testScores.verbal === 'object'
          ? studentProfile.testScores.verbal.score * 2 // convert /5 → /10
          : studentProfile.testScores.verbal ?? 0,
      color: '#1E3A8A',
    },
              ...
              <div key={area.key} className="relative">
                <div className={`border-2 rounded-xl p-6 transition-all ${getBadgeOpacity(area.score)}`}>
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {area.score !== null
                      ? `${Math.round((area.score / 10) * 100)}% completion`
                      : 'Not assessed yet'}
                  </p>
                </div>
            ))}
